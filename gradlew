...
# Need to resolve the link for realpath on non-GNU systems
while [ -h "$app_path" ]; do
...
if [ "$cygwin" = "false" ] && [ "$darwin" = "false" ] && [ "$nonstop" = "false" ]; then
    case $(ulimit -H -n) in
      "")
        # This happens when ulimit is not supported
        ;;
      soft)
        MAX_FD=65536
        ;;
...
# Collect all arguments for the java command, following the shell quoting and evaluation rules
